...
|---|---|---|---|---|---|---|---|
| L-001 | Rapid architecture churn in LLM harness layer | PRs #34, #60, #72, #80, #92 repeatedly reworked harness/API strategy across `harness/` and `harness_responses/` | Core interfaces and migration boundaries were not frozen before feature expansion | Define a versioned harness contract and migration plan in WD spec before implementation; no parallel harness expansion without explicit deprecation plan | WD Team | in-progress | High-maintenance churn driver |
