...
|---|---|---|---|---|---|---|---|
| L-001 | Rapid architecture churn in LLM harness layer | PRs #34, #60, #72, #80, #92 repeatedly reworked harness/API strategy across `harness/` and `harness_responses/` | Core interfaces and migration boundaries were not frozen before feature expansion | Define a versioned harness contract and migration plan in WD spec before implementation; no parallel harness expansion without explicit deprecation plan | WD Team | in-progress | M2 enforced single harness runtime path and explicit legacy-toggle rejection. |
| L-002 | Phase/state contract instability in orchestrator flow | PRs #24, #40, #52, #73, #82, #85, #90 repeatedly modified phase behavior, validation gates, and scene flow | Pipeline state machine was not treated as a strict contract with acceptance tests per phase | Make phase transitions deterministic and schema-validated (`project_state` contract + phase-specific acceptance tests) | WD Team | in-progress | M1 deterministic transitions + M5 downstream media prerequisite gates. |
| L-003 | Self-heal loops became primary path instead of fallback | PR #55 explicitly audits parse failure loop; PRs #40 and #73 target loop convergence and repair behavior | First-pass generation quality/contracts were too weak; retries compensated for design gaps; oversized retry payloads hit model context ceilings, truncating critical diagnostics | In WD, optimize first-pass correctness, cap retries, and enforce structured, size-bounded retry payloads (error codes + minimal diffs) | WD Team | in-progress | M4 added loop-signature detection/no-blind-retry blocking; M6 added structured failure events and blocked trace bundles. |
| L-004 | Scaffold/slot contract drift in generated scenes | PRs #4, #51, #52, #58, #73 focus on scaffold generation fixes, duplicate helper removal, and stricter validation | Scaffold boundaries were mutable and not continuously verified | Introduce immutable scaffold markers plus automated contract tests that run before build/repair phases | WD Team | in-progress | Core generator integrity issue |
| L-005 | Prompt/schema drift caused parsing and serialization failures | PR #72 (JSON responses), #76 (timing validation), #77 (triple-quote corruption), #85/#89/#90 (prompt/schema alignment/layout validation) | Prompt contracts and parser expectations evolved independently | Single source of truth for phase schemas and prompt manifests; CI check that prompt outputs map exactly to parser/schema contracts | WD Team | in-progress | M2 enforced canonical phase schema binding; M3 added manifest contracts + CI alignment checks. |
| L-006 | Oversized multi-concern PRs increased regression risk | PRs #60, #92, #80, #42, #44 changed large surfaces (50-100 files) across docs/scripts/harness/examples at once | Changes were not scoped to single concern/milestone | Enforce smaller milestone PR slices in WD plan; one architectural concern per PR with explicit rollback boundary | WD Team | in-progress | M6 added lessons-linked regression matrix + CI policy checks to keep changes reviewable and traceable. |
| L-007 | Documentation drift and stale policy references | PRs #2, #33, #36, #79, #93 repeatedly corrected contradictions/stale references | Docs were updated reactively after implementation changes | Treat WD tech spec wiki as authoritative and gate merges on docs alignment for affected phases | WD Team | implemented | M6 enforces canonical docs-as-gate policy in CI (`check_docs_as_gate.py` + workflow baseline). |
| L-008 | Voice/TTS policy and implementation churn | PRs #21, #23, #36, #60 repeatedly migrated TTS strategy/providers and references | Voice subsystem boundaries and policy were not stabilized early | Freeze WD voice policy early (service contract, fallback policy, cache behavior) and isolate provider adapters behind one interface | WD Team | implemented | M5 delivered deterministic voice adapter, cache-key contract, metadata sidecars, and explicit fallback gating. |
| L-009 | Validation layers accreted without simplifying upstream design | PRs #39, #40, #41, #76, #85, #90 add more semantic/layout/timing validation layers over time | Defensive checks grew faster than root-cause generator fixes | Define WD validation hierarchy with explicit ownership: schema -> semantic -> runtime; each added gate must remove a known failure class | WD Team | in-progress | M4 encoded gate order/ownership; M5 added render/assembly contract gates; M6 logs gate ownership/signatures for diagnosable escalation. |
| L-010 | Repository mixed runtime artifacts/examples with core system evolution | Multiple PRs (for example #14, #42, #58, #60, #82) include `projects/`/`examples/` artifacts alongside core logic updates | Diagnostics artifacts and core code were not cleanly separated | Separate WD operational artifacts from product code/docs; require clean boundaries for test fixtures vs runtime outputs | WD Team | in-progress | M5 added artifact manifests; M6 added blocked trace bundles with explicit artifact pointers and retry history summaries. |
...
| L-019 | Unspecified API/ helper constraints led to invalid kwargs and misuse | Runtime errors include unsupported kwargs (`numbers_with_elaborate_zero`, `stroke_dash`, `max_tip_length`, `max_stroke_width`), invalid axis config keys (`include_numbers`, `numbers_with_elongated_ticks`), `adaptive_title_position` missing `content_group`, and `self.camera.frame` on non-moving camera scenes (see `docs/analysis/fh-pr-audit/flaming-horse-generated/gibbs-phenomenon/log/error.log`; repeated `stroke_dash` failures in `scene_04` repair trace) | LLMs guessed API surface and helper signatures without a version-locked contract | In WD, expose a strict, versioned API surface and helper signatures in prompts or via deterministic wrappers; add linting to reject unsupported kwargs before runtime | WD Team | planned | Reduces runtime failures from API drift and guesswork |
| L-020 | Repair loops repeated ineffective LaTeX fixes without explicit remediation guidance | `docs/analysis/fh-pr-audit/flaming-horse-generated/gibbs-phenomenon/log/conversation.log` shows repeated `scene_03` repairs that preserved double-escaped MathTex despite LaTeX errors | Repair prompts did not include concrete, failure-class-specific remediation steps (for example “normalize LaTeX escapes before emitting raw strings”) | In WD, repair prompts must include failure-class-specific remediation directives and a required diff checklist for repeat failures | WD Team | planned | Prevents loops that restate the same broken pattern |
| L-021 | Plot callbacks assumed vectorized input, breaking `Axes.plot` | `scene_06` and `scene_08` repairs used vectorized functions that failed when Manim passed scalar inputs (see `docs/analysis/fh-pr-audit/flaming-horse-generated/gibbs-phenomenon/log/error.log`) | Generation did not enforce scalar-compatible plot callbacks; no validation gate checked callback signature/behavior | In WD, enforce scalar-compatible plot callbacks or wrap vectorized functions; add validation and tests that reject non-scalar-safe callbacks | WD Team | planned | Prevents repeated runtime failures in plotting-heavy scenes |
| L-022 | Plan outputs contained invalid Manim API names that propagated into scene code | `docs/analysis/fh-pr-audit/flaming-horse-generated/topic-1/plan.json` includes `ShowPassingThrough(...)`; `docs/analysis/fh-pr-audit/flaming-horse-generated/topic-1/log/error.log` shows `NameError: name 'ShowPassingThrough' is not defined` in `scene_03.py` | Plan format allowed code-like API calls without validation; build phase treated them as authoritative | In WD, plan `visual_ideas` must be natural-language intent or constrained tags; run plan lint to reject unknown Manim API tokens before build | WD Team | planned | Prevents invalid API leakage from planning to build |
| L-023 | MathTex token indexing caused brittle scene failures | `docs/analysis/fh-pr-audit/flaming-horse-generated/topic-1/log/error.log` shows `IndexError: list index out of range` from `sub_eq[2]` in `scene_03.py` | Generated code assumed stable submobject indices for MathTex, which is not guaranteed | In WD, require `get_part_by_tex` (or explicit `Tex` parts) for MathTex subobject references; add lint to reject numeric indexing of MathTex | WD Team | planned | Reduces fragile text-part access failures |
...
- None unresolved for Phase 4 closure. Prior items were resolved in approved tech-spec decisions and Phase 4 implementation-plan baselines.
...
- Added L-017 through L-021 based on gibbs-phenomenon instruction-quality vs output analysis and repair traces.
- Added L-022 through L-023 based on instruction-quality vs output analysis for other FH runs.

### 2026-03-01

- Updated lesson execution status to `in-progress` for active Phase 5 implementation baseline.
- M5 completion evidence recorded; L-008 moved to `implemented` with voice adapter/cache/fallback policy contracts.
- M6 completion evidence recorded; L-007 moved to `implemented` with CI-enforced docs-as-gate policy.
- Reconciled prior open questions into approved tech-spec and implementation-plan decisions for Phase 4 completion.
